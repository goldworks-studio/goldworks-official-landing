...
The homepage and all default legal pages are in English, including accessible labels, navigation, email request templates and search/sharing metadata. The homepage’s central promise is “Crafting play. Making it matter.” The shared privacy policy offers a language dropdown with English, Spanish, Japanese, Korean, Brazilian Portuguese and Simplified Chinese, using native language names without flags. Browser language never changes the English default. Autonyms inside language controls are the only non-English text allowed on default legal pages.
...
- Shared policy translations: `/privacy/` (English), `/es/privacy/` (Spanish), `/ja/privacy/` (Japanese), `/ko/privacy/` (Korean), `/pt-br/privacy/` (Brazilian Portuguese), `/zh-cn/privacy/` (Simplified Chinese).
- `/ko/privacy/byeolmong/` and `/ko/account-deletion/` preserve the existing Korean service notice and deletion instructions. These related documents currently offer English and Korean in their dropdowns; other policy translations explicitly label links to English documents. The dropdown never offers a translation that does not exist.
- All policy versions are static, server-rendered pages with canonical URLs, language alternatives and sitemap entries. The selector uses native keyboard-accessible controls and Next.js navigation; without JavaScript, direct language links remain available. No translation widget, visitor text transmission, language cookie, geolocation or automatic redirect is used.
- English and the four added translations share `components/privacy-document.tsx` and the copy dictionaries in `lib/privacy-translations/`. Korean retains its existing wording. `check:translations` verifies all 71 text entries, 10 contents labels and a hash of the English source. If the source copy changes, review every translation before updating its `sourceDigest`; never refresh the hashes without reviewing the text. The export check verifies all 11 routes, the available and selected dropdown options, no-JavaScript links and language metadata.

### Language review

Google Play’s published privacy-policy requirements do not specify Korean as a mandatory language. This is not a general exemption from local disclosure requirements. The PIPC’s April 2026 guidance, printed page 12 (PDF page 14), discusses clear, understandable notices. Language options help users read the applicable disclosures; they do not determine the operator’s jurisdiction or establish compliance. The app can link directly to an appropriate localized notice. In-app notice and consent obligations remain separate.

Localization preserves the September 10, 2026 effective date and the disclosed processing practices. The general introduction refers to applicable privacy and data protection laws; regional complaint resources remain in the contact section. Original wording remains in Git history.
